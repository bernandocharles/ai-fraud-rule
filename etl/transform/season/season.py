...
spark = SparkSession.builder.appName("FDS AI - Season").getOrCreate()
input_path = sys.argv[1]
output_path = sys.argv[2]

#Define the data path
src_path = f"file://{input_path}/transfer.csv"
target_path = f"file://{output_path}/season"
...
df = df.withColumn("months", F.month("trx_date"))
...
spark.sql("""select *, RANK () OVER (PARTITION BY account_id, months ORDER BY transaction_date ASC) trx_dec
		from trf
    where months = "12"
""").createOrReplaceTempView("number_trx_dec")


spark.sql("""select a.*, CASE WHEN b.trx_dec IS NULL THEN 0 ELSE b.trx_dec END AS trx_dec 
from trf a
left join number_trx_dec b
ON a.id = b.id
""").createOrReplaceTempView("season") 
...
query = spark.sql("""SELECT DISTINCT
                    ...
                    CASE WHEN b.trx_dec IS NULL THEN 0 ELSE b.trx_dec END AS trx_dec
                      from trf a
                      left join number_trx_dec b
                      ON a.id = b.id
                  """)
