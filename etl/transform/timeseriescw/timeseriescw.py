...
spark = SparkSession.builder.appName("FDS AI - TimeseriesCW").getOrCreate()
input_path = sys.argv[1]
output_path = sys.argv[2]

#Define the data path
src_path = f"file://{input_path}/tarik_tunai.csv"
target_path = f"file://{output_path}/timeseriescw"
...
spark.sql("""select id, account_id, ts as ts1m, count(*) as trx1m, sum(transaction_amount) as amt1m from
    (select a.id, a.account_id, last_date, ts, ts1m, ts5m, ts1h, ts1d, dates, transaction_amount from 
        (select id, account_id, transaction_date as last_date, ts, ts1m, ts5m, ts1h, ts1d from data1) a 
        CROSS JOIN 
...
spark.sql("""select id, account_id, ts as ts5m, count(*) as trx5m, sum(transaction_amount) as amt5m from
    (select a.id, a.account_id, last_date, ts, ts1m, ts5m, ts1h, ts1d, dates, transaction_amount from 
        (select id, account_id, transaction_date as last_date, ts, ts1m, ts5m, ts1h, ts1d from data1) a 
        CROSS JOIN 
        (select id, account_id, ts as dates, transaction_amount from data1) b
...
spark.sql("""select id, account_id, ts as ts1h, count(*) as trx1h, sum(transaction_amount) as amt1h from
    (select a.id, a.account_id, last_date, ts, ts1m, ts5m, ts1h, ts1d, dates, transaction_amount from 
        (select id, account_id, transaction_date as last_date, ts, ts1m, ts5m, ts1h, ts1d from data1) a 
        CROSS JOIN 
        (select id, account_id, ts as dates, transaction_amount from data1) b
...
spark.sql("""select id, account_id, ts as ts1d, count(*) as trx1d, sum(transaction_amount) as amt1d from
    (select a.id, a.account_id, last_date, ts, ts1m, ts5m, ts1h, ts1d, dates, transaction_amount from 
        (select id, account_id, transaction_date as last_date, ts, ts1m, ts5m, ts1h, ts1d from data1) a 
        CROSS JOIN 
        (select id, account_id, ts as dates, transaction_amount from data1) b
...
query = spark.sql("""
                  SELECT DISTINCT
                    ...
                    CASE WHEN c.trx5m IS NULL THEN 0 ELSE c.trx5m END AS trx5m,
