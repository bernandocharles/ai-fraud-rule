...
spark = SparkSession.builder.appName("FDS AI - InOutCW").getOrCreate()
input_path = sys.argv[1]
output_path = sys.argv[2]
#Define the data path
trf_path = f"file://{input_path}/transfer.csv"
cw_path = f"file://{input_path}/tarik_tunai.csv"
target_path = f"file://{output_path}/inoutcw"

print(f"checking source path in {trf_path} and {cw_path}")
print(f"checking target path in {target_path}")

# Compatibility script for spark version 2 running on spark 3
spark.sql("set spark.sql.legacy.timeParserPolicy=LEGACY")

dftrf = spark.read.csv(trf_path, sep=",", header="true", inferSchema= 'true', timestampFormat="yyyy-MM-dd hh:mm:ss")
dfcw = spark.read.csv(cw_path, sep=",", header="true", inferSchema= 'true', timestampFormat="yyyy-MM-dd hh:mm:ss")


dftrf = dftrf.withColumn("trx_date", 
...
dfcw = dfcw.withColumn("trx_date", 
        F.when(F.col("transaction_date").rlike("\d{4}-\d{2}-\d{2}"), F.to_timestamp("transaction_date", 'yyyy-MM-dd hh:mm:ss'))
         .when(F.col("transaction_date").rlike("\d{4}-\d{2}-\d{2} \d{1,2}:\d{2}:\d{1,2}"), F.to_timestamp("transaction_date", 'yyyy-MM-dd hh:mm:ss'))
    )


dftrf = dftrf.withColumn("dates", F.to_date("trx_date", 'yyyy-MM-dd hh:mm:ss'))

dftrf.createOrReplaceTempView("trf_ts")

dfcw = dfcw.withColumn("dates", F.to_date("trx_date", 'yyyy-MM-dd hh:mm:ss'))

dfcw.createOrReplaceTempView("cw_ts")
...
spark.sql("""select id, account_id, ts as ts5m, count(*) as trx5m, sum(transaction_amount) as amtout5m from
    (select a.id, a.account_id, last_date, ts, ts5m, ts30m, ts1h, dates, transaction_amount from 
...
(select a.id as id, a.account_id as account_id, a.last_date, a.ts as ts, a.ts5m, a.ts30m, a.ts1h, b.id as bid, b.account_id as baccid, b.dest_account_id as bdestaccid, b.dates, b.transaction_amount from
        (select id, account_id, transaction_date as last_date, ts, ts5m, ts30m, ts1h from data1) a 
        CROSS JOIN 
        (select id, account_id, dest_account_id, ts as dates, transaction_amount from data2) b
