...
spark = SparkSession.builder.appName("FDS AI - Small").getOrCreate()
input_path = sys.argv[1]
output_path = sys.argv[2]

#Define the data path
src_path = f"file://{input_path}/transfer.csv"
target_path = f"file://{output_path}/small"
...
spark.sql("""select *, RANK () OVER (PARTITION BY account_id, dates ORDER BY transaction_date ASC) trx_5k
		from trf
		where transaction_amount <= 5000
""").createOrReplaceTempView("trx_small_5k")

spark.sql("""select *, RANK () OVER (PARTITION BY account_id, dates ORDER BY transaction_date ASC) trx_10k
		from trf
		where transaction_amount <= 10000
""").createOrReplaceTempView("trx_small_10k")

spark.sql("""select a.*, CASE WHEN b.trx_5k IS NULL THEN 0 ELSE b.trx_5k END AS trx_5k, CASE WHEN c.trx_10k IS NULL THEN 0 ELSE c.trx_10k END AS trx_10k 
from trf a
left join trx_small_5k b
ON a.id = b.id
left join trx_small_10k c
ON a.id = c.id
...
query = spark.sql("""SELECT DISTINCT
                    ...
                    CASE WHEN b.trx_5k IS NULL THEN 0 ELSE b.trx_5k END AS trx_5k, 
                    CASE WHEN c.trx_10k IS NULL THEN 0 ELSE c.trx_10k END AS trx_10k 
                      from trf a
                      left join trx_small_5k b
                      ON a.id = b.id
                      left join trx_small_10k c
