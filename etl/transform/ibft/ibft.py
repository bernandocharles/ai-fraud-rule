...
spark = SparkSession.builder.appName("FDS AI - IBFT").getOrCreate()
input_path = sys.argv[1]
output_path = sys.argv[2]

#Define the data path
src_path = f"file://{input_path}/transfer.csv"
target_path = f"file://{output_path}/ibft"
...
spark.sql("""select *, RANK () OVER (PARTITION BY account_id, dates ORDER BY transaction_date ASC) ibft_out
		from trf
		where account_bank = "Sumut" and dest_bank != "Sumut"
""").createOrReplaceTempView("trx_ibft_out")

spark.sql("""select *, RANK () OVER (PARTITION BY dest_account_id, dates ORDER BY transaction_date ASC) ibft_in
		from trf
		where account_bank != "Sumut" and dest_bank = "Sumut"
""").createOrReplaceTempView("trx_ibft_in")

spark.sql("""select a.*, CASE WHEN b.ibft_out IS NULL THEN 0 ELSE b.ibft_out END AS ibft_out, 
                         CASE WHEN c.ibft_in IS NULL THEN 0 ELSE c.ibft_in END AS ibft_in 
from trf a
left join trx_ibft_out b
ON a.id = b.id
left join trx_ibft_in c
ON a.id = c.id
""").createOrReplaceTempView("ibft") 
...
query = spark.sql("""SELECT DISTINCT
                    ...
                    CASE WHEN b.ibft_out IS NULL THEN 0 ELSE b.ibft_out END AS ibft_out, 
                    CASE WHEN c.ibft_in IS NULL THEN 0 ELSE c.ibft_in END AS ibft_in 
                      from trf a
                      left join trx_ibft_out b
                      ON a.id = b.id
