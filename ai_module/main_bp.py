...
import classes_bp
import utils_bp
from classes_bp import data_processor, MCD, ISO, ISOMCD, models, RF, rfmodels, clustering, dtmodels, DT
from utils_bp import target_names, value, unit, type, source, filters
import os
import sys
...
dirs = f"{input_path}/bill_payment.csv"
...
if rows_count == 0:
    ...
    if out.shape[0] < 10:
        ...
        fraud_rule = json.loads(json_rule)
        print(fraud_rule)
        json_status = '{"status": "No anomalies detected for bill payment"}'
        print(json_status)
        ...
        fraud_rule = dtinstance.training(dfo, k, new_columns, sm_tree_depths, sm_min_split, sm_min_leaf, utils_bp.target_names, utils_bp.type, utils_bp.source, utils_bp.unit, utils_bp.value, utils_bp.filters, scale)

    print("---Fraud Rules---")
    print(f"===\n {fraud_rule}")

with open(f'{output_path}/bill_payment.json','w') as f:
    rule_json = json.dumps(fraud_rule)
    print(rule_json, file=f)

with open(f'{output_path}/bill_payment_status.json','w') as f:
