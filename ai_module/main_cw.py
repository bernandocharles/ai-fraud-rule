...
import classes_cw
import utils_cw
from classes_cw import data_processor, MCD, ISO, ISOMCD, models, RF, rfmodels, clustering, dtmodels, DT
from utils_cw import target_names, value, unit, type, source, filters
import os
import sys
...
dirs = f"{input_path}/tarik_tunai.csv"
...
if rows_count == 0:
    ...
    if out.shape[0] < 10:
        ...
        fraud_rule = json.loads(json_rule)
        print(fraud_rule)
        json_status = '{"status": "No anomalies detected for cash withdrawal"}'
        print(json_status)
        ...
        fraud_rule = dtinstance.training(dfo, k, new_columns, sm_tree_depths, sm_min_split, sm_min_leaf, utils_cw.target_names, utils_cw.type, utils_cw.source, utils_cw.unit, utils_cw.value, utils_cw.filters, scale)

    print("---Fraud Rules---")
    print(f"===\n {fraud_rule}")

with open(f'{output_path}/tarik_tunai.json','w') as f:
    rule_json = json.dumps(fraud_rule)
    print(rule_json, file=f)

with open(f'{output_path}/tarik_tunai_status.json','w') as f:
