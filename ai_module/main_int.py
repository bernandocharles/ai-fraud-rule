...
import classes_int
import utils_int
from classes_int import data_processor, MCD, ISO, ISOMCD, models, RF, rfmodels, clustering, dtmodels, DT
from utils_int import target_names, value, unit, type, source, filters
import os
import sys
...
dirs = f"{input_path}/transaksi_internal.csv"
...
if rows_count == 0:
    ...
    if out.shape[0] < 10:
        ...
        fraud_rule = json.loads(json_rule)
        print(fraud_rule)
        json_status = '{"status": "No anomalies detected for internal transaction"}'
        print(json_status)
        ...
        fraud_rule = dtinstance.training(dfo, k, new_columns, sm_tree_depths, sm_min_split, sm_min_leaf, utils_int.target_names, utils_int.type, utils_int.source, utils_int.unit, utils_int.value, utils_int.filters, scale)

    print("---Fraud Rules---")
    print(f"===\n {fraud_rule}")

with open(f'{output_path}/transaksi_internal.json','w') as f:
    rule_json = json.dumps(fraud_rule)
    print(rule_json, file=f)

with open(f'{output_path}/transaksi_internal_status.json','w') as f:
