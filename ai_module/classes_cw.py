...
class data_processor:
    ...
    def fit_mcd_preprocessing(
        ...
        df["trxcw1m"] = df["trxcw1m"].fillna(0)
        df["trxcw5m"] = df["trxcw5m"].fillna(0)
        df["trxcw1h"] = df["trxcw1h"].fillna(0)
        df["trxcw1d"] = df["trxcw1d"].fillna(0)
        #df["dest1m"] = df["dest1m"].fillna(0)
        #df["dest30m"] = df["dest30m"].fillna(0)

        df["inoutcw5m"] = df["inoutcw5m"] + 0.001
        df["inoutcw30m"] = df["inoutcw30m"] + 0.001
        df["inoutcw1h"] = df["inoutcw1h"] + 0.001

        df["inoutcw5m"] = self.func_inout(df, "inoutcw5m")
        df["inoutcw30m"] = self.func_inout(df, "inoutcw30m")
        df["inoutcw1h"] = self.func_inout(df, "inoutcw1h")
    ...
    def get_rules(self, tree, feature_names, class_names, type, source, unit, value, filters, scalers):
        ...
        def recurse(node, path, paths):
            ...
            if tree_.feature[node] != _tree.TREE_UNDEFINED:
                ...
                if name == "inoutcw5m" or name == "inoutcw30m" or name == "inoutcw1h":
                    if thres == 300000:
                        thresholdn = 1
                    else:
                        try:
