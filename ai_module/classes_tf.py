...
class data_processor:
    ...
    def fit_mcd_preprocessing(
        ...
        df["trx30s"] = df["trx30s"].fillna(0)
        df["trx1m"] = df["trx1m"].fillna(0)
        df["trx30m"] = df["trx30m"].fillna(0)
        df["trx1h"] = df["trx1h"].fillna(0)
        #df["dest1m"] = df["dest1m"].fillna(0)
        #df["dest30m"] = df["dest30m"].fillna(0)

        df["inout5m"] = df["inout5m"] + 0.001
        df["inout30m"] = df["inout30m"] + 0.001
        df["inout1h"] = df["inout1h"] + 0.001

        df["inout5m"] = self.func_inout(df, "inout5m")
        df["inout30m"] = self.func_inout(df, "inout30m")
        df["inout1h"] = self.func_inout(df, "inout1h")
